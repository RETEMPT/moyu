...
- 自动模式按问题关键词在 L1 伴读、L2 学情、L3 跨库之间分级，也支持手动切换；
- L3 使用本地 Markdown 分块与关键词检索，回答显示命中来源数量；
- 未配置模型服务时明确以离线模式运行，不伪造在线回答；
- 模型档案和对话记录已预留独立 Preferences 存储契约，OpenAI 兼容 Provider 接口已隔离在 `services/ai/`；
- 网络调用、流式 SSE、会话侧栏和真实模型设置面板仍按 `AI_AGENT_PLAN.md` 的后续阶段接入。
